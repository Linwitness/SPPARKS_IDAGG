...
path  = os.getcwd() + "/"
if math.isnan(core):
    shell_file =  "agg_poly_1Core.in"
    core_tag = "_singleCore"
    IC_tag = ""
else:
    shell_file = "agg_poly_multiCore.in"
    ...
    for k in range(len(inc_delta)):
        for m in range(len(reference_axis)):
            for n in range(len(random_seed)):
                fileBase="p_"+tri_type+"_"+base_name[i]+core_tag+\
                ...
                ICfile ="VoronoiIC_"+base_name[i]+"_elong"+IC_tag
                ...
                if os.path.exists(path+fileBase+".dump") and IWannaOnlyPlot:
                    ...
                    de.plot_energy_video(timestep, energy_figure, figure_path, 0.6)
                elif IWannaReplot:
                    file_path = fileBase
                    grain_structure_figure = np.load(path + "results/"+file_path+".npy")
                    timestep = 30 * np.array(range(len(grain_structure_figure)))
                    de.plot_structure_video(timestep, grain_structure_figure, "video/"+file_path+"_structure")
                else:
                    ...
                    figure_path="video/"+figure_name2
                    timestep, energy_figure = de.dump2energy(path+fileBase, 161)
                    ...
                    if not os.path.exists("results/"+fileBase+".npy"):
                        timestep, grain_structure_figure = de.dump2img(path+fileBase, 161)
